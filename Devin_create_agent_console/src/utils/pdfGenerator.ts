...
import { DXProposal as DXProposalSchema } from '../mastra/utils/companyResearch';
import { z } from 'zod';

type DXProposal = z.infer<typeof DXProposalSchema>;

/**
 * Generate a PDF file from a DX proposal
 * @param proposal DX proposal object
 * @param outputPath Path to save the PDF file
 * @returns Path to the generated PDF file
 */
export async function generateDXProposalPDF(proposal: DXProposal, outputPath?: string): Promise<string> {
 ...
 */
function formatBusinessChallenges(challenges: string[] | any[]): string {
 ...
 * @param opportunities DX opportunities
 * @returns Formatted string
 */
 ...
 * @param roadmap Implementation roadmap
 * @returns Formatted string
 */
function formatRoadmap(roadmap: any[]): string {
  ...
  return roadmap.map((phase, index) => {
    ...
    if (phase.deliverables && phase.deliverables.length > 0) {
      ...
      text += `  タスク:\n`;
      phase.tasks.forEach((item: string) => {
        text += `    • ${item}\n`;
      });
    }
 ...
 * @param investment Investment summary
 * @returns Formatted string
 */
function formatInvestmentSummary(investment: any): string {
  if (!investment) {
    return '情報なし';
  }
